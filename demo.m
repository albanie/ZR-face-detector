...
% name = 'multipie_independent';

data = load(fullfile(paths.DATA_ROOT, 'models', 'zhu-ramanan', ...
    sprintf('%s.mat', name)));
model = data.model;

disp('Model visualization');
components = 1:numel(model.components);
visualizemodel(model, components, name);

% my modification
componentImgPaths = getImgsInDir('figs/', 'jpg');
componentImgs = cellfun(@imread, componentImgPaths, 'UniformOutput', false);

% make the images a consistent size
componentImgs = cellfun(@(x) imresize(x, [400 400]), componentImgs, ...
    'UniformOutput', false);
componentImgs = cat(4,componentImgs{:});
clf; figure(1);
vl_imarraysc(componentImgs);
